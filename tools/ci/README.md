...
The workflow builds the AtomZero game shell (the `core/` directory and
`icon.svg` only — see the [Technical Design](../../doc/AtomZero_Technical_Design.md))
for five platforms and publishes the artifacts to a GitHub Release.
...
1. **`prepare`** — resolves the version (from the pushed tag or manual
   input), runs `generate_export_presets.py`, and uploads the generated
   `export_presets.cfg` as an artifact.
2. **`build-macos`** — installs Godot 4.6.3 + export templates on macOS,
   exports an `.app` bundle, and zips it.
3. **`build-linux`** — exports the Linux binary, then packages it as both
   `.deb` and `.rpm` via `package_linux.sh`.
4. **`build-windows`** — cross-exports a Windows `.exe` from the Linux
   editor and zips it with its `.pck`/`.dll` siblings.
5. **`build-android`** — sets up Java 17, the Android SDK (API 34) and NDK
   r23c, configures signing, and exports a signed `.apk`.
6. **`build-ios`** — exports an Xcode project (unsigned by default) and
   builds a `.ipa` via `xcodebuild`, or directly exports a signed `.ipa`
   when signing secrets are provided.
7. **`release`** — downloads all build artifacts, stages them, and creates
   a GitHub Release with `softprops/action-gh-release`.
8. **`notify-failure`** — runs `if: always()` after every build; if any
   build failed, it creates a GitHub issue (and posts to the webhook).
...
- **Push of a version tag** matching `v*` (e.g. `v2026.6.30`). The version
  is derived by stripping the leading `v`, and a Release is always created.
- **Manual dispatch** (`workflow_dispatch`) with two inputs:
  - `version` — the version string to build (default `2026.6.30`).
  - `create_release` — whether to create a GitHub Release after the builds
    succeed (default `true`).

```bash
# Tag-triggered build (recommended for releases)
git tag v2026.6.30
git push origin v2026.6.30

# Manual build without a release
gh workflow run build.yml -f version=2026.6.30 -f create_release=false
```
...
field in [`project.godot`](../../project.godot) (currently `2026.6.30`),
...
artifact filename (e.g. `AtomZero-2026.6.30-macos.zip`). The `release` job
...
diff export_presets.cfg /tmp/export_presets.cfg   # should be empty

# Preview the failure-notification issue body
python3 tools/ci/notify.py --dry-run --version 2026.6.30 --failed-jobs macos,linux

# Build a .deb from a local Linux binary
bash tools/ci/package_linux.sh deb 2026.6.30 dist/AtomZero.x86_64
```
